...
> что ломает компрессию и SkipScan. Если колонка содержит много NULL —
> не используй её как segmentby или оставь segmentby пустым.
> Цель: **100–10 000 строк на сегмент** внутри одного чанка.

### Полностью отключить
*Важно: перед выполнением все чанки должны быть переведены в строчный формат!*
```sql
...
### Конвертировать ВСЕ чанки в строчный формат
```sql
DO $$
...
WHERE j.proc_name = 'policy_columnstore';
```
